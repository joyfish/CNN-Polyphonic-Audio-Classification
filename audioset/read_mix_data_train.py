...
dataset_folder_path = '/cs/tmp/pmh20/a/'#ExtraWork/AugmentedDataset/TestSet/diffClusters/'
new_path = '/cs/tmp/pmh20/a/'#ExtraWork/AugmentedDataset/TestSet/diffClusters/'
...
for folder in os.listdir(dataset_folder_path):
    for cls in os.listdir(dataset_folder_path + folder):
        if ".DS_Store" not in cls:
            for filename in os.listdir(dataset_folder_path + folder + "/" + cls):
                if ".DS_Store" not in filename:
                    ...
                    try:
                        fts = vggish_input.wavfile_to_examples(dataset_folder_path + folder + "/" + cls + "/" + filename)
                    except: 1                    
#for feat in fts:
...
with open(new_path + "test_" + folder + ".txt", "wb") as fp:
    pickle.dump((x, y), fp)
