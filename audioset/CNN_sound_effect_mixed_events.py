...
def _test_model(predictions, vLabels, sess, classes):
    ...
    for i, l in enumerate(vLabels):
        ...
        if all(vLabels[i] == classification[i]):
            ...
            comparison.append(0)
    conf_matrix = sess.run(tf.confusion_matrix(correctLabels, predictedLabels, num_classes=20))
    print(conf_matrix)
...
def main(_):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        with tf.variable_scope('mymodel'):
            ...
            fc2 = slim.fully_connected(fc, 40, activation_fn=nn_ops.relu)
        ...
        trainTestSplit = read_in_dataset._get_train_test_split_saved_mixed(repeat=False)
